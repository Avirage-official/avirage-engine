...
function calculateCodeMatch(
  ...
  for (const patternId of codeReq.corePatterns) {
    ...
    if (patterns[patternId]) {
      ...
      score += patternMatch.confidence * 1.0;
      coreMatches.push(patternMatch);
    }
  }

  // Check minimum core pattern requirement
  if (score < codeReq.minimumCoreMatch) {
  ...
  return {
    ...
    matchScore: Math.max(0, matchPercentage), // Don't go negative
...
export function matchCulturalCodes(patterns: DetectedPatterns): {
  ...
  const matches: CulturalCodeMatch[] = [];

  // Calculate match for each code
  for (const codeReq of CODE_REQUIREMENTS) {
    const match = calculateCodeMatch(codeReq, patterns);
    if (match) {
      matches.push(match);
    }
  }
  ...
  return {
    primary: matches[0] || createDefaultMatch(1),
    secondary: matches[1] || createDefaultMatch(2),
    tertiary: matches[2] || createDefaultMatch(3),
    allMatches: matches,
  };
}

/**
 * Create a default match if not enough codes qualify
 */
function createDefaultMatch(rank: number): CulturalCodeMatch {
  return {
    codeId: 0,
    codeName: "Namsea", // Fallback to Namsea (most moderate)
    fullName: "Vietnamese + Thai Fusion",
    matchScore: 50,
    matchPercentage: 50,
    confidence: "low",
